...
def get_corners_over_threshold(sobel_edge, kernel_size, std_dev, thresh):
    ...
    gaussian_kernel = GaussianBlur.get_gaussian_kernel(kernel_size, std_dev)
...
def get_corner_score_maximums(sobel_edge, corner_scores):
    ...
    for x in range(int(CannyEdge.CANNY_RADIUS), corner_scores.shape[0] - int(CannyEdge.CANNY_RADIUS)):
        for y in range(int(CannyEdge.CANNY_RADIUS), corner_scores.shape[1] - int(CannyEdge.CANNY_RADIUS)):
...
def get_neighboring_edge_pixel_mags(corner_scores, pixel, perp_angle):
    xComp = CannyEdge.CANNY_RADIUS * cos(perp_angle)
    yComp = CannyEdge.CANNY_RADIUS * sin(perp_angle)
    dx = 0
    dy = 0
    if yComp > -CannyEdge.CANNY_VAL_45_DEG and yComp < CannyEdge.CANNY_VAL_45_DEG:
        dy = 0
    elif yComp >= CannyEdge.CANNY_VAL_45_DEG:
        dy = 1
    elif yComp <= -CannyEdge.CANNY_VAL_45_DEG:
        dy = -1
        
    if xComp > -CannyEdge.CANNY_VAL_45_DEG and xComp < CannyEdge.CANNY_VAL_45_DEG:
        dx = 0
    elif xComp >= CannyEdge.CANNY_VAL_45_DEG:
        dx = 1
    elif xComp <= -CannyEdge.CANNY_VAL_45_DEG:
        dx = -1
