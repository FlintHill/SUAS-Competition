...
function pullMissionInfo() {
	console.log("trigger pullMissionInfo()");
...
function interopConnect() {
	interopEndpointURL = "http://" + $("#address").val() + ":8000/" + $("#endpointURL").val();

	console.log(interopEndpointURL);

	$.when($.ajax({
		method: "POST", type: "POST", // redundancy
		url: (interopEndpointURL + "login"),

		async: true, cache: false, crossDomain: true,
		...
		error:function(textStatus) {
			console.log("Failed to connect to interop server at: " + interopEndpointURL + "login");
...
function onMessage(evt) {
	...
	$("#velocity").html((JSON.parse(evt.data).velocity).toFixed(2));
