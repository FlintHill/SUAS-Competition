...
class TargetWithBackgroundCreator(object):
    ...
    @staticmethod
    def create_specified_target_with_random_background(shape_type, shape_orientation, letter, size, proportionality, shape_color, letter_color, path_to_backgrounds, rotation):
        """
        ...
        background = BackgroundGenerator(target_image.width, target_image.height, path_to_backgrounds).generate_background()

        random_x_min = random.randint(10, background.width - target_image.width - 20)
        random_y_min = random.randint(10, background.height - target_image.height - 20)

        cropped_background = background.crop((random_x_min, random_y_min,
                                              random_x_min + target_image.width + 20,
                                              random_y_min + target_image.height + 20))

        cropped_background.paste(target_image, (10, 10), target_image)
        return cropped_background

    @staticmethod
    def create_specified_target_with_specified_background(shape_type, shape_orientation, letter, size, proportionality, shape_color, letter_color, path_to_background, rotation):
        """
    ...
    @staticmethod
    def create_random_target_with_random_background(size_range, proportionality_range, path_to_backgrounds):
        """
    ...
    @staticmethod
    def create_random_target_with_specified_background(size_range, proportionality_range, path_to_background):
        target_image = RandomTargetCreator.create_random_target(size_range, proportionality_range)
        background = Image.open(path_to_background)
