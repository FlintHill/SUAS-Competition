...
class ModularTargetWithoutBackground(object):
    ...
    def create_modular_target_without_background(self):
    ...
    def record_modular_target_without_background(self, index_number):
        data={}
        data["targets"] = []
        data["targets"].append({
            "shape_type": self.modular_target.shape_type,
        ...
        with open(Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY + "/modular_single_targets_without_background_answers/" + str(index_number) + ".json", 'w') as outfile:
            json.dump(data, outfile, indent=4)

        self.background.save(Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY + "/modular_single_targets_without_background/" + str(index_number) + ".png")
