...
class ModularSyntheticDatasetMaker(object):
    ...
    @staticmethod
    def run_modular_single_targets_without_background_generator(number_of_single_targets_without_background, process_number):
        for index in range(number_of_single_targets_without_background):
            single_target = ModularTargetWithoutBackground()
            single_target.create_modular_target_without_background()
            Logger.log("Current process: " + str((process_number / number_of_single_targets_without_background) + 1) + "\n")
            single_target.record_modular_target_without_background(process_number + index + 1)

    @staticmethod
    def create_modular_single_targets_without_background(number_of_single_targets_without_background):
        if (os.path.isdir(Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY)):
            pass
        else:
            os.mkdir(Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY)

        if (os.path.isdir(Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY + "/modular_single_targets_without_background")):
            raise Exception("Cannot create Modular Single Targets Without Background: Save directory already exists")
        if (os.path.isdir(Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY + "/modular_single_targets_without_background_answers")):
            raise Exception("Cannot create Modular Single Targets Without Background Answers: Save directory already exists")

        os.mkdir(Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY + "/modular_single_targets_without_background")
        os.mkdir(Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY + "/modular_single_targets_without_background_answers")
        ...
        for index in range(cpu_count):
            starting_index = index * int(pics_per_process)
            image_generation_process = multiprocessing.Process(target=ModularSyntheticDatasetMaker.run_modular_single_targets_without_background_generator, args=(pics_per_process, starting_index))
            jobs.append(image_generation_process)
            image_generation_process.start()
        ...
        Logger.log("Modular Single Targets Without Background saved at: " + Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY + "/modular_single_targets_without_background")
        Logger.log("Modular Single Targets Without Background Answers saved at: " + Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY + "/modular_single_targets_without_background_answers\n")

        print("====================================")
        print("Total number of modular single targets without background generated:", len(os.listdir(Settings.SAVE_PATH + Settings.ANSWERS_DIRECTORY + "/modular_single_targets_without_background")))
        print("Total elapsed time (sec):", timeit.default_timer() - start_time)
        print("====================================")
