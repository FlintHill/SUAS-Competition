...
class SWTScrubber(object):
    ...
    def scrub(cls, filepath):
        ...
        shapes, shape_h, shape_w = cls._connect_components(swt)
        swts, heights, widths, topleft_pts, images = cls._find_letters(swt, shapes, shape_h, shape_w)
    ...
    def _create_derivative(cls, filepath):
        ...
        #theta = np.arctan2(sobelx_8u, sobely_8u)
        if diagnostics:
            cv2.imwrite('edges.jpg',edges)
            #cv2.imwrite('sobelx_8u.jpg', sobelx_8u)
            #cv2.imwrite('sobelx_8u.jpg', sobely_8u)
        ...
        #return (edges, sobelx_8u, sobelx_8u, theta)
    ...
    def _swt(self, theta, edges, sobelx64f, sobely64f):
        ...
        for index_1 in range(len(mag_g)):
            for index_2 in range(len(mag_g[index_1])):
                if (mag_g[index_1][index_2] == 0):
                    mag_g[index_1][index_2] = 1

        grad_x_g = step_x_g / mag_g
        grad_y_g = step_y_g / mag_g

        #sobelx64f = -1 * sobelx64f
        #sobely64f = -1 * sobely64f
        #mag_g = np.sqrt( sobelx64f * sobelx64f + sobely64f * sobely64f )
        #grad_x_g = sobelx64f / mag_g
        #grad_y_g = sobely64f / mag_g
        ...
        for x in xrange(edges.shape[1]):
            for y in xrange(edges.shape[0]):
                if edges[y, x] > 0:
                    step_x = step_x_g[y, x]
                    step_y = step_y_g[y, x]
                    #step_x = -1 * sobelx64f[y, x]
                    #step_y = -1 * sobely64f[y, x]
                    #mag = mag_g[y, x]
                    #mag = np.sqrt( sobelx64f[y, x] * sobelx64f[y, x] + sobely64f[y, x] * sobely64f[y, x] )
                    #grad_x = -1 * sobelx64f[y, x]/mag
                    #grad_y = -1 * sobely64f[y, x]/mag
                    grad_x = grad_x_g[y, x]
                    grad_y = grad_y_g[y, x]
                    ray = []
                    ray.append((x, y))
                        ...
                        if cur_x != prev_x or cur_y != prev_y:
                            ...
                            try:
                                if edges[cur_y, cur_x] > 0:
                                    ...
                                    angle = grad_x * -grad_x_g[cur_y, cur_x] + grad_y * -grad_y_g[cur_y, cur_x]
                                    print angle

                                    if angle > 1.0 or angle < -1.0:
                                        if np.absolute(angle)-1<1e-10:
                                            dec_, int_ = math.modf(angle)
                                            angle = int_
                                    if math.acos(angle)< np.pi/2.0:
