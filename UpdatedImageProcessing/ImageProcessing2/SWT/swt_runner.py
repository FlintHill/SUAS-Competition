    ...
    def run_swt(image_path):
        ...
        for x, y, width, height in swt_result:
            for i in xrange(0, 3): # just to make lines thicker
                rr, cc = SWT_Runner.rectangle_perimeter(y + i, x + i, height, width, shape=image.shape, clip=True)
                image[rr, cc] = (255, 0, 0)

        imshow(image)
        #msave("result.png", image)
        plt.show()
