...
class SDAConverter(object):
    ...
    def set_uav_position(self, new_position, bearing):
        """
        Set the UAV's current position within the map
        """
        haversine_distance = haversine(GPSCoordinates(self.get_initial_coordinates().get_latitude(), self.get_initial_coordinates().get_longitude(), self.get_initial_coordinates().get_altitude()), GPSCoordinates(new_position.lat, new_position.lon, new_position.alt))

        dy = update_data.get_haversine_distance() * sin(bearing)
        dx = update_data.get_haversine_distance() * cos(bearing)
        new_drone_location = np.array([dx, dy])
    ...
    def set_current_waypoint(self, waypoint):
        """
        Set the current waypoint of the UAV

        :param waypoint: The waypoint to set for the UAV
        :type waypoint: LocationGlobalRelative
        """
        pass

    def get_current_wapoint(self):
        """
        Returns the current waypoint
        """
        pass

    def get_curr_waypoint_distance(self):
        """
        Return the distance (in meters) to the current waypoint
        """
        pass
