...
class TargetDirection(object):
    ...
    def init_direction(self):
        ...
        for i in range(0, len(possible_values)):
            proximity_angles = AngleRounder.get_compass_angles_sorted_by_proximity_to_angle(get_angle_of_2d_vector(possible_values[i]))
    ...
    def get_max_closest_to_possible_values(self, possible_values):
        ...
        smallest_index = 0
        for i in range(0, len(possible_values)):
            sorted_differences = sorted(self.maximums, key=lambda radius_angle : abs(get_angle_between_vectors(radius_angle.get_unit_vector(), possible_values[i])))
            difference = abs(get_angle_between_vectors(sorted_differences[0].get_unit_vector(), possible_values[i]))
