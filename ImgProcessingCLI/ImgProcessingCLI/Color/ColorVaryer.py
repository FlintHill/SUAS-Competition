...
class ColorVaryer:
    ...
    def __init__(self, img, image):
        ...
        self.mean_blur_img = Image.fromarray(cv2.blur(numpy.array(self.img), (ColorVaryer.KERNEL_SIZE, ColorVaryer.KERNEL_SIZE)))#MeanBlur.get_mean_blurred_rgb_img(self.img, self.image, ColorVaryer.KERNEL_SIZE)
    ...
    def init_variance_img(self):
        mean_mean_blurred_img = Image.fromarray(cv2.blur(numpy.array(self.mean_sub_img), (ColorVaryer.VARIANCE_KERNEL_SIZE, ColorVaryer.VARIANCE_KERNEL_SIZE)))#MeanBlur.get_mean_blurred_bw_img(self.mean_sub_img, self.mean_sub_img.load(), ColorVaryer.VARIANCE_KERNEL_SIZE)
        ...
        blur_variance_img = Image.fromarray(cv2.blur(numpy.array(self.variance_img), (ColorVaryer.BLUR_VARIANCE_KERNEL_SIZE, ColorVaryer.BLUR_VARIANCE_KERNEL_SIZE)))#MeanBlur.get_mean_blurred_bw_img(self.variance_img, self.variance_img.load(), ColorVaryer.BLUR_VARIANCE_KERNEL_SIZE)# GaussianBlur.get_gaussian_filtered_bw_img(self.variance_img, self.variance_img.load(), 7, 3.0)
